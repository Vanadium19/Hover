...
public class Engine : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (Physics.Raycast(_transform.position, forward, out RaycastHit hitInfo, _maxDistance, _layerMask))
        {
    ...
    {
        _engineWorldSpeed = _transform.position - _engineWorldOldPosition;
        float dotResult = Mathf.Clamp01(Vector3.Dot(forceDirection.normalized, _engineWorldSpeed.normalized));
        _rigidbody.AddForceAtPosition(-forceDirection * _engineWorldSpeed.magnitude * dotResult * _damping, 
            _transform.position, ForceMode.Force);
